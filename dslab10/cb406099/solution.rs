...
impl Raft {
    ...
    async fn become_leader(&mut self) {
        self.reset_timer(self.config.election_timeout/10);
        ...
        if self.enabled {
            ...
            match (&mut self.process_type, msg.content) {
                ...
                (_, RaftMessageContent::RequestVote { candidate_id }) => {
                    ...
                    if self.config.self_id != candidate_id {
                        if self.state.voted_for == None && self.state.current_term <= msg.header.term {
                    ...
                    self.sender.send(&candidate_id, RaftMessage {
                        header: RaftMessageHeader {
                            term: self.state.current_term,
                        },
